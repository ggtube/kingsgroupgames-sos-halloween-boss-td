...
Class.Mixin(Screen, {
	...
	tween(tween_name, targets, next, options) {

		if (!this.isArray(targets)) targets = [targets];

		if (!options) options = {};

		let tween_props = this.Tweens[tween_name];
		...
		if (!tween_props.name) tween_props.name = (this.Name === 'CallToAction' ? 'end-screen' : this.Name).toLowerCase();

		let tween_object = options.tweenObject || {};
		...
		this._tweens.push(tween_object);

		return tween_object;

	},

	_tween(target, tween_object, options) {
		...
		if (!tween_props.to) this._tweenEnd(tween_object);
			...
			this.each(tween_props.to, to_object => {
				...
				if (this.isObject(to_object[1])) {
					...
					if (target[to_object[0]] && target[to_object[0]].set) {
						...
						if (!this.isArray(to_object[1])) to_object[1] = [to_object[1], to_object[1], to_object[1]];

						if (to_object[1][0] || to_object[1][0] === 0) tween_vars.x = this.calculate(to_object[1][0], {fixedMultiplier: 1, specialMultiplier: 1, initialValue: target.params[to_object[0]] ? target.params[to_object[0]][0] : 0});
						if (to_object[1][1] || to_object[1][1] === 0) tween_vars.y = this.calculate(to_object[1][1], {fixedMultiplier: 1, specialMultiplier: 1, initialValue: target.params[to_object[0]] ? target.params[to_object[0]][1] : 0});
						if (to_object[1][2] || to_object[1][2] === 0) tween_vars.z = this.calculate(to_object[1][2], {fixedMultiplier: 1, specialMultiplier: 1, initialValue: target.params[to_object[0]] ? target.params[to_object[0]][2] : 0});
						...
						if (options.save && to_object[0] in target.params) {
							...
							if (this.isArray(to_object[1])) {
								...
								if (to_object[1][2] !== null) target.params[to_object[0]][2] = to_object[1][2];
	...
	_tweenSet(props, target) {
		...
		this.each(props, function(set_object) {
			...
			if (target[set_object[0]] && target[set_object[0]].set) {
				...
				if (!this.isArray(set_object[1])) set_object[1] = [set_object[1], set_object[1], set_object[1]];

				x = (set_object[1][0] || set_object[1][0] === 0) ? this.calculate(set_object[1][0], {fixedMultiplier: 1, specialMultiplier: 1, initialValue: target.params[set_object[0]] ? target.params[set_object[0]][0] : 0}) : target[set_object[0]].x;
				y = (set_object[1][1] || set_object[1][1] === 0) ? this.calculate(set_object[1][1], {fixedMultiplier: 1, specialMultiplier: 1, initialValue: target.params[set_object[0]] ? target.params[set_object[0]][1] : 0}) : target[set_object[0]].y;
				z = (set_object[1][2] || set_object[1][2] === 0) ? this.calculate(set_object[1][2], {fixedMultiplier: 1, specialMultiplier: 1, initialValue: target.params[set_object[0]] ? target.params[set_object[0]][2] : 0}) : target[set_object[0]].z;

				target[set_object[0]].set(x, y, z);
	...
	_tweenProperty(tween_object, target, props, time, delay, ease, options) {

		if (!options) options = {};

		if (tween_object.props.name && Settings['tween-factor-' + tween_object.props.name]) time /= Settings['tween-factor-' + tween_object.props.name] || 1;
		else time /= Settings['tween-factor-other'] || 1;
		...
		let tween = TweenLite.to(target, time / 1000, props);

		props.onComplete = function() {

			if (tween && !tween._pc_completed) {
	...
	_tweenEnd(tween_object) {

		let is_completed = true;

		for (let i=0; tween_object.tweens[i]; i++) {
		...
		if (is_completed) {
			...
			let _end = function() {
				...
				if (tween_object.props.next) {
					...
					let loop = tween_object.startingProps.loop,
	...
	stopTween(tween_object) {
		...
		if (tween_object) {
			...
			this._tweens = this._tweens.filter((to) => {return to !== tween_object;});
	...
	stopAllTweens() {
		...
		this.each(this._tweens, (tween_object) => {

			this.stopTween(tween_object);
	...
	visualiseBezierPath(sprite, splines_params) {

		if (!sprite._tweenBesierVisualiser) sprite._tweenBesierVisualiser = this.buildChild(sprite.parent, {type: 'graphics'});

		let segments = BezierPlugin.bezierThrough(splines_params.values, splines_params.curviness, true);
