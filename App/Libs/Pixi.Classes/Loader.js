...
let Loader = {
	...
	load() {
		...
		if (!MRAID.Settings["game-code-url"]) {
			...
			Loader.loadScript(Settings['assets-path'] + Settings["game-code-url"] + (Settings["version"] ? "?v=" + Settings["version"] : ''), function() {
	...
	extendStyles() {
		...
		MRAID.extendStyles(Loader.textEl, Settings["loading-text-styles"]);
		...
		MRAID.extendStyles(Loader.logoPublisher, Settings["loading-icon-publisher-styles"]);
