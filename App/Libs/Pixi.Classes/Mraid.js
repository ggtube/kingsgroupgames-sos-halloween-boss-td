...
const MRAID = {
	...
	showApp() {
		...
		MRAID.EndCard = Settings['end-card'] ? App.EndCard : App.CallToAction;
	...
	getLocale() {
		...
		let locale = (navigator.userLanguage || navigator.language || navigator.browserLanguage || navigator.systemLanguage || '').replace('-', '_').toLowerCase().substring(0, 2);
	...
	processSettings(settings, values) {
		...
		if (!settings["assets-path"]["value"] && !settings["assets-path"]["default"]) {
		...
		const track_actions = ['track-google-analytics-actions', 'track-mixpanel-actions', 'track-libraries'];
	...
	duplicateSettingsAssetsForDashboard(name, asset, values) {
		...
		for (let i=0; i<values.length; i++) Settings.Assets[name + '-' + values[i]] = {type: asset["type"], url: asset["url"].replace("?", values[i]), compress: asset["compress"], quality: asset["quality"]};
	...
	showCloseButton() {
		...
		this.closeButtonContainer.addEventListener("click", () => {
			...
			if (App.Video && App.Video.showed) {
				...
				if (Settings["cta-only"]) MRAID.EndCard.show();
		...
		if (Settings["track-google-analytics-actions"] && event in Settings["track-google-analytics-actions"] && (!this.TrackedEvents[event] || Settings["track-google-analytics-actions"][event+':multiple'])) {
			...
			for (i=0; Settings["track-google-analytics-actions"][event][i]; i++) {
				...
				if (MRAID.isViewable || Settings["track-events-on-viewable"] === false) {
					...
					this.trackSendActionGoogleAnalytics(event, Settings["track-google-analytics-actions"][event][i], params);

				} else {

					this.HoldEvents.GoogleAnalytics.push([event, Settings["track-google-analytics-actions"][event][i], params]);
	...
	trackSendActionGoogleAnalytics(event, action, params) {

		if (Settings["track-google-analytics-key"]) MRAID.log('Track Google Analytics:', event, '###', action, params);

		if (Settings["track-google-analytics-key"] && window.ga) {

			ga.apply(window, action.concat([params]));
