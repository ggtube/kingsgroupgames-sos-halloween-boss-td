...
var Screen = new Class({
	...
	buildChilds: function(childs, container, is_reposition) {
		...
		this.each(childs, function(child_params) {
			...
			let child = this.buildChild(container, child_params, is_reposition);
	...
	resize: function(force = false) {
		...
		this.each(this._containers, function(container) {
			...
			if (child_params.type === 'container' || child_params.type === 'three-ui') {

				let child = this[child_params.name];

				if (child) {

					if (App.Engine === 'Pixi') this.applyParams(child, child.params, false, false, !force);
					else if (App.Engine === 'Three') this.applyThreeParams(child, child.params);

				}

				if (child_params.childs) this.resizeChilds(child.children, force);
	...
	/**
	 ...
	 @method resizeChilds
	 */
	resizeChilds: function(childs, force = false) {

		this.each(childs, function(child) {
			...
			if (child.params.type !== 'three-viewport') this.resizeChilds(child.children);
	...
	resume: function() {
		...
		/**
		 en: Fired then current screen resumes. In the start of this event name you need to specify Screen {{#crossLink "Screen/Name:property"}}{{/crossLink}} value
		 ru: Вызывается когда текущий скрин снимается с паузы. Перед названием этого события всегда нужно писать {{#crossLink "Screen/Name:property"}}{{/crossLink}} экземпляра Screen

		 @event resumed
		 */
		Broadcast.call(this.Name + ' resumed', arguments);
	...
	pause: function() {
		...
		/**
		 en: Fired then current screen pauses. In the start of this event name you need to specify Screen {{#crossLink "Screen/Name:property"}}{{/crossLink}} value
		 ru: Вызывается когда текущий скрин ставится на паузу. Перед названием этого события всегда нужно писать {{#crossLink "Screen/Name:property"}}{{/crossLink}} экземпляра Screen

		 @event paused
		 */
		Broadcast.call(this.Name + ' paused');
