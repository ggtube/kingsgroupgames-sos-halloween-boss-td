...
Class.Mixin(Screen, {
	...
	SpecialParams: ['name', 'type', 'image', 'scaleStrategy', 'scaleGlobal', 'tint', 'mask', 'draw', 'styles', 'hit', 'filters', 'event', 'button', 'spineData', 'spineAtlas', 'spineTexture', 'video', 'text', 'childs'],

	DisableParamsReset: [],

	initialize() {

		this._jsmpeg_sprites = [];

		Broadcast.on(this.Name + ' update', () => {

			this.each(this._jsmpeg_sprites, child => {

				//safari sometimes return negative currentTime
				if (child.video.currentTime < 0) child.video.currentTime = 0;

				if (child.video.currentTime !== child.textureUpdateTime) {

					child.textureUpdateTime = child.video.currentTime;

					child.videoSprite.texture.update();

				}

				//2020.04.07 Предотвращаем рассинхронизацию видео и звука
				if (child.sound) {

					let currentTime = child.sound.seek();

					if (Math.abs(currentTime - child.video.currentTime) > 0.1) {

						child.sound.seek(child.video.currentTime);
				...
				if (child.playTillTime) {

					if (child.video.currentTime >= child.playTillTime) {

						child.playTillTime = null;

						child.pause();

						if (child.playTillNext) {

							child.playTillNext();

							child.playTillNext = null;
		...
		Broadcast.on(this.Name + ' before build', () => {
			...
			this.each(this.Containers, function(container_params) {
				...
				if (container_params.type === 'container') {
					...
					let container = this.buildChild(App.Stage, container_params);
	...
	buildChild: function(container, child_params) {
		...
		if (child_params.type === 'sprite') {
		...
		}  else if (child_params.type === '9-slice') {

			// PIXI.NineSlicePlane
			// https://pixijs.download/dev/docs/PIXI.NineSlicePlane.html

			child = new PIXI.NineSlicePlane(this.getTexture(child_params.frame || child_params.image), child_params.leftWidth || 10, child_params.topHeight || 10, child_params.rightWidth || 10, child_params.bottomHeight || 10);

			if (child_params.size) {
				child.width = child_params.size[0];
				child.height = child_params.size[1];
			}
		...
		} else if (child_params.type === 'text-chars') {

			child = new PIXI.TextChars(child_params.text, child_params.styles);
			...
			if (child_params.frameTemplate) {
				...
				if (!child_params.frameStep) child_params.frameStep = 1;

				if ("frameStart" in child_params) {

					if (child_params.frameStart <= child_params.frameEnd && child_params.reverse) {

						for (let i = child_params.frameStart; i <= child_params.frameEnd; i = i + child_params.frameStep) {
						...
						for (let i = child_params.frameEnd - 1; i >= child_params.frameStart; i = i - child_params.frameStep) {

							if (!child_params.frameFilter || child_params.frameFilter(i)) child_params.frames.push(child_params.frameTemplate.replace('???', i < 100 ? (i < 10 ? '00' + i : '0' + i) : i).replace('??', i < 10 ? '0' + i : i).replace('?', i));

						}

					} else if (child_params.frameStart <= child_params.frameEnd) {
					...
					for (let i = 0; i <= child_params.frameNumbers.length; i++) {

						let c = child_params.frameNumbers[i];
			...
			let frames = _.map(child_params.frames, (image) => {

				return this.getTexture(image);

			});

			child = new PIXI.AnimatedSprite(frames);
			...
			if (child_params.src) {
				...
				child.video = child.texture.baseTexture.resource.source;

			} else if (child_params.video) {

				let video = child_params.video;

				if (typeof video === 'string' && App.Assets[video]) video = App.Assets[video].video;

				let source;
				let asset = App.Assets[child_params.video];

				if (asset["jsmpeg"]) source = video.renderer.canvas;
				else source = video;

				child = new PIXI.Container(); // wrapper for auto video scale (video resolution independent)

				const sprite = new PIXI.Sprite(PIXI.Texture.from(source));

				child.videoSprite = sprite;

				//2020.03.08 смещение видео при канвас рендерере (в firefox и chrome одинаково) HTML5 видео тег размеры видео берёт другие - anchor 0.5 работает не верно
				if (asset.width && asset.height && App.Renderer.type === PIXI.RENDERER_TYPE.CANVAS) {

					child.videoSprite.anchor.set(0, 0);
					child.videoSprite.position.set(-asset.width/2, -asset.height/2);

				} else {

					child.videoSprite.anchor.set(0.5);

				}

				if (asset["jsmpeg"]) {

					if (!asset["jsmpegScale"]) asset["jsmpegScale"] = 1;

					let scale = this.isArray(asset["jsmpegScale"]) ? asset["jsmpegScale"] : [asset["jsmpegScale"], asset["jsmpegScale"]];

					// 2020.04.01 jsmpeg видео перевёрнуто если используется webgl на macos и ios safari (включая WebView который не определяется как safari)
					// MRAID.ios - означает в том числе и MacOS судя по коду Mraid.js 2020.04.01
					if (App.Renderer.type === PIXI.RENDERER_TYPE.WEBGL && MRAID.ios) scale[1] *= -1;

					child.videoSprite.scale.set(...scale);

				} else if (!asset["jsmpeg"] && asset["scale"]) {

					let scale = this.isArray(asset["scale"]) ? asset["scale"] : [asset["scale"], asset["scale"]];
				...
				if (asset["jsmpeg"]) this._jsmpeg_sprites.push(child);
			...
			child.play = function(time, next) {
				...
				let video = child_params.video;
				let asset = App.Assets[video];

				child.video.play();

				if (asset && asset["jsmpeg"]) {
					child.sound = App.Assets[video + ':sound'].sound;
					if (child.soundInstanceId) child.sound.seek((typeof time === 'number') ? time : child.video.currentTime).play(child.soundInstanceId);
					else child.soundInstanceId = child.sound.seek((typeof time === 'number') ? time : child.video.currentTime).play();
				}

				if (typeof time === 'number') child.video.currentTime = time;

				//Помечаем что пользователь начал воспроизводить видео либо программно оно начало воспроизводиться
				//Это нужно чтобы первоначальное проигрывание видео с целью загрузки шло без звука и не включался звук при первом клике и других условиях общих
				child.video.userPlayOccured = true;
				child.video.muted = !MRAID.firstInteractionOccurred;

				if (child.sound) {
					child.sound.userPlayOccured = true;
					child.sound.mute(!MRAID.firstInteractionOccurred);
				}
			...
			child.playTill = function(seconds, next) {

				child.playTillTime = seconds;
				child.playTillNext = next;
			...
			child.pause = function() {
				...
				MRAID.log(child.name, 'child.pause', child.videoSprite ? child.videoSprite.texture.baseTexture.resource.source.baseURI : (child.texture.baseTexture.source.src || child.texture.baseTexture.source.firstChild.src));

				child.video.pause();
				if (child.sound) child.sound.pause();
			...
			if (App.Assets[child_params.video].jsmpeg) {

				child.video.options.onEnded = () => {

					child.isPlaying = false;

					child.video.currentTime = 0;

					endFuncs.forEach(endFunc => endFunc.call(this));

				};

				child.video.addEventListener = function(event, func) {

					if (event === 'ended') child.onEnd(func);

				}

			} else {

				child.video.addEventListener('ended', () => {
			...
			let spine_atlas_asset = App.Assets[child_params.spineAtlas] || App.Assets[child_params.spineAtlas + '-parsed'];

			if (spine_atlas_asset.type === 'text') {

				spine_atlas = new PIXI.spine.core.TextureAtlas();

				let spineAtlasLoader = new PIXI.spine.core.AtlasAttachmentLoader(spine_atlas);
				let spineJsonParser = new PIXI.spine.core.SkeletonJson(spineAtlasLoader);

				let spineData = spineJsonParser.readSkeletonData(spine_atlas_asset.text);

				child = new PIXI.spine.Spine(spineData);

				container.addChild(child);

			} else if (spine_atlas_asset.type === 'atlas') {

				spine_atlas = new PIXI.spine.core.TextureAtlas();

				let all_textures = {};

				this.each(spine_atlas_asset.data.data.frames || spine_atlas_asset.json.frames, function(frame_data, frame_name) {
				...
				let spineData = spineJsonParser.readSkeletonData(App.Assets[child_params.spineData].data);
	...
	getTextures: function(names) {
		...
		for (let i=0; names[i]; i++) textures.push(this.getTexture(names[i]));

		return textures;

	},

	applyParams: function(child, params, is_change_params, is_rebuild, is_reset) {
		...
		this.each(params, (param_value, param_name) => {

			//Except custom values
			if (this.contains(this.SpecialParams, param_name)) return;

			//Disable params reset on resize
			if (is_reset && this.DisableParamsReset && (this.DisableParamsReset === true || this.contains(this.DisableParamsReset, param_name))) return;
		...
		if (params.image) {
			...
			let texture = this.getTexture(params.image);
		...
		if ('scaleStrategy' in params) child.scale.set(this.getScaleByStrategy(params.scaleStrategy) || 1);
		...
		if (params.mask) {
			...
			if (['rect', 'circle', 'arc'].includes(params.mask[0])) {
				...
				if (!child.mask) {
					...
					child.mask.texture = PIXI.Texture.from(child.mask.texture.baseTexture.source);
		...
		if (params.text) this.setText(child, params.text, is_change_params, params.noWarnings);
		...
		if (child.applyParams) child.applyParams();
	...
	enableEvents: function (sprite, event_params) {
		...
		var on_down_call = function() {
			...
			if (event_params.pulse) {

				if (!this.isArray(event_params.pulse)) event_params.pulse = [event_params.pulse];

				this.pulse(sprite, ...event_params.pulse);
			}
