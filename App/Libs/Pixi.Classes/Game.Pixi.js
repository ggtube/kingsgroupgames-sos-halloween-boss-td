	...
	*/
	...
	createPixi() {
		...
		if (this.IsCreateGame) {
			...
			if (this.World) this.createThreeWorld(this.World);
	...
	checkPixiSoundsMute() {
		...
		MRAID.log('checkPixiSoundsMute', Settings["sounds-mute"], MRAID.isViewable, MRAID.firstInteractionOccurred);

		if (window.Howler) {

			if (Settings["sounds-mute"] === "viewable" && !MRAID.isViewable) return this.muteSounds();
