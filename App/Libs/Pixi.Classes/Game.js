...
const Game = new Class({
	...
	prepareEngine() {
		...
		if (this.Engine === 'Three' || this.World) this.prepareThreeEngine();
	...
	loadImages(loader, assets) {
		...
		if (this.Engine === 'Three' || this.World) this.loadThreeImages(loader, assets);
	...
	loadAtlases(loader, assets) {
		...
		if (this.Engine === 'Three') this.loadThreeAtlases(loader, assets);
	...
	loadSources(loader) {
		...
		if (this.Engine === 'Three') this.loadThreeSources(loader);
	...
	create() {
		...
		if (this.Engine === 'Three') this.createThree();
	...
	injectRenderer() {
		...
		if (this.Engine === 'Three') this.injectThreeRenderer();
	...
	startTicker() {
		...
		if (this.Engine === 'Three') this.startThreeTicker();
	...
	update() {
		...
		if (this.World && this.World.clampTimeOffset) this.timeOffset = Math.max(16, Math.min(50, this.timeOffset));
		...
		if (this.Engine === 'Three' || this.World) this.updateThree();
	...
	resize(force = false) {
		...
		if (this.Engine === 'Three' || this.World) this.resizeThree(force);
	...
	getAssetQuality(asset) {
		...
		if (asset["compress"] === false) return false;
