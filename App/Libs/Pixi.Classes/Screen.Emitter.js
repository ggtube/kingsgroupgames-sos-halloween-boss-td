...
Class.Mixin(Screen, {
	...
	initialize: function() {
		...
		this.SpecialParams = _.uniq((this.SpecialParams || []).concat(['images', 'limit', 'count', 'ticks', 'interval', 'particlePosition', 'particleSpeed', 'particleAcceleration', 'particleTween']));
	...
	updateEmitters: function() {
		...
		this.updateEmitterParticlesPosition(App.timeOffset / 1000);
		...
		for (let i=0; (i < emitter.waveCount) && (!emitter.paused) && (emitter.particlesLimit > emitter.particlesCount); i++) {
			...
			if (typeof particle_tweens === 'object') {
				...
				var particle_tween = this.tween(particle_tweens, sprite, function(tween_object) {
					...
					if (tween_object.targets[0].destroy) tween_object.targets[0].destroy();
					else if (tween_object.targets[0].parent.remove) tween_object.targets[0].parent.remove(tween_object.targets[0]);
